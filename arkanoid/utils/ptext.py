...
def getsurf(text, fontname=None, fontsize=None, width=None, widthem=None,
            color=None,
            background=None, antialias=True, ocolor=None, owidth=None,
            scolor=None, shadow=None,
            gcolor=None, alpha=1.0, align=None, lineheight=None, angle=0,
            cache=True):
    ...
    ocolor = None if owidth is None else _resolvecolor(ocolor,
                                                       DEFAULT_OUTLINE_COLOR)
    scolor = None if shadow is None else _resolvecolor(scolor,
    ...
    if angle:
        surf0 = getsurf(text, fontname, fontsize, width, widthem, color,
                        background, antialias,
                        ocolor, owidth, scolor, shadow, gcolor, alpha, align,
                        lineheight, cache=cache)
        ...
        surf0 = getsurf(text, fontname, fontsize, width, widthem, color,
                        background, antialias,
                        ocolor, owidth, scolor, shadow, gcolor=gcolor,
                        align=align,
        ...
        if len(color) > 3 and color[3] == 0:
            ...
            array[x0:x0 + w0, y0:y0 + h0] -= array0.clip(
                max=array[x0:x0 + w0, y0:y0 + h0])
            del array, array0
        else:
        ...
        if len(color) > 3 and color[3] == 0:
            array = pygame.surfarray.pixels_alpha(surf)
            array0 = pygame.surfarray.pixels_alpha(surf0)
            array[opx:-opx, opx:-opx] -= array0.clip(
                max=array[opx:-opx, opx:-opx])
            del array, array0
        else:
        ...
        if background is None or (len(background) > 3 and background[
            ...
            lsurfs = [font.render(text, antialias, color).convert_alpha() for
                      text in texts]
        else:
            lsurfs = [
                font.render(text, antialias, color, background).convert_alpha()
                for text in texts]
        if gcolor is not None:
            ...
            for lsurf in lsurfs:
                ...
                for j in (0, 1, 2):
                    array[:, :, j] = (
                    (1.0 - m) * array[:, :, j] + m * gcolor[j]).astype(
                        array.dtype)
                del array
...
def draw(text, pos=None,
    ...
    tsurf = getsurf(text, fontname, fontsize, width, widthem, color,
                    background, antialias,
                    ocolor, owidth, scolor, shadow, gcolor, alpha, align,
